...
class CVAE(object):
    ...
    def __init__(self, img_shape, nd_latent, lr=4e-4, flatten=True):
        ...
        logits = self._decoder(self.z, condition_ph)
    ...
    def _encoder(self, x, c):
        ...
        with tf.variable_scope('encoder'):
            ...
            x = tf.layers.batch_normalization(xc,training=self.is_training)
    ...
    def train(self, dataset, mb_size=3):
        ...
        while step < 50000:
            ...
            fd_map = {self.obs_ph: obs, self.condition_ph: labels, self.target_ph: obs, self.is_training: True}
    ...
    def eval(self):
        ...
        img = self.sess.run(self.recon, feed_dict={self.z: z, self.condition_ph: c, self.is_training: False})
        for _img in img:
            plt.imshow(_img[...,0])
            plt.show()
